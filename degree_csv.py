...
if not os.path.exists('csv/degree_csv'):
        os.mkdir('csv/degree_csv')
...
for f in glob.glob('./degree/*.xls'):
    ...
    if df_cnt % 100 == 0 or f == glob.glob('./degree/*.xls')[-1]:
        df_end = re.findall("\d+", str(f))[0]
        pd.concat(df_list).to_csv('csv/degree_csv/degree_' + df_start + '-' + df_end + '.csv', sep=',')
        print('degree_' + df_start + '-' + df_end + '.csv saved')
        df_list = []
    df_cnt += 1

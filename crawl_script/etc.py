...
def load_etc(driver, page, pageScale, keyword, StartCount):

    url = 'http://www.riss.kr/search/Search.do?detailSearch=false&viewYn=OP&queryText=&iGroupView=5&icate=all&colName=re_t&pageScale=500&strSort=RANK&order=%2FDESC&query='
    url2 = '&iStartCount='
...
if __name__ == '__main__':
    ...
    try:
        F = open('etc_startcount.txt', 'r')
        StartCount = int(F.readline())
    except:
        StartCount = 0
    pageScale = 100

    driver.get('http://www.riss.kr/search/Search.do?detailSearch=false&viewYn=OP&queryText=&iGroupView=5&icate=all&colName=re_t&pageScale=500&strSort=RANK&order=%2FDESC&query=' + keyword + '&iStartCount=' + str(StartCount))
    ...
    for i in range(ceil(results / pageScale)):
        ...
        with open('etc_startcount.txt', 'w') as F:
            F.write(str(StartCount + i * pageScale))
        load_etc(driver, i, pageScale, keyword, StartCount)
        for filename in os.listdir(os.path.join(os.path.join(os.environ['USERPROFILE']), 'Downloads')):
            if filename.startswith("myCabinet") and ".crdownload" not in filename:
                try:
                    os.rename(os.path.join(os.path.join(os.environ['USERPROFILE']), 'Downloads', filename), os.path.join("../etc", "etc_data"+str(StartCount + i * pageScale) + filename[-4:]))
                except:
                    os.rename(os.path.join(os.path.join(os.environ['USERPROFILE']), 'Downloads', filename), os.path.join("../etc", "etc_data"+str(StartCount + i * pageScale + 1) + filename[-4:]))

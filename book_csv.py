...
if not os.path.exists('csv/book_csv'):
        os.mkdir('csv/book_csv')
...
for f in glob.glob('./book/*.xls'):
    ...
    if (df_cnt % 100 == 0) or (f == glob.glob('./book/*.xls')[-1]):
        df_end = re.findall("\d+", str(f))[0]
        pd.concat(df_list).to_csv('csv/book_csv/book_' + df_start + '-' + df_end + '.csv', sep=',')
        print('book_' + df_start + '-' + df_end + '.csv saved')
        df_list = []
    df_cnt += 1
